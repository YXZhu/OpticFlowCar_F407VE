...
__IO uint16_t Distance_data=0;//保存测距数据
...
//	LCD_ShowString(30,140+90,200,16,16,"Distance:    0 mm");//显示测量距离 
	while(1)
	{
		 ...
		 if(Status==VL53L0X_ERROR_NONE)
		 {
			 ...
			 {
				re = 0;
				tt[0] = Distance_data>>8;
				tt[1] = Distance_data&0xff;
				HAL_UART_Transmit(&huart1,tt,2,10);
			 }
			 else delay_ms(10);
			// printf("%04x",Distance_data);
		 }
		 else
		 delay_ms(10);
...
//			 switch(mode)
//			 {
//				 case Default_Mode0:  LCD_ShowString(95,140+95,300,16,16, "Default        "); break;//默认
//				 case HIGH_ACCURACY1: LCD_ShowString(95,140+95,300,16,16, "High Accuracy  "); break;//高精度
