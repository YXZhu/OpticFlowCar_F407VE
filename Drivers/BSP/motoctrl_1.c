...
CCMRAM int8_t spdC = 60;  // 设置车子速度 // 0 ~ 120
...
//		}
		...
		if(moto_run%5 == 0)  // 每10x5ms执行一次
		{
			if(angle.i != 1500) // 差速计算
			{
				...
				spdA_t = spdC*(1+LIMIT(error,-1.5f,0));
				spdB_t = spdC*(1-LIMIT(error,0,1.5f));
		...
		if(moto_run%60 == 0) // 电机不转 转速置零 每60x5ms执行一次
		{
...
static void Moto_TimeInit(void)
{
	...
	/* 轨迹控制PID参数 */
	straightctrlpid.Kp = 2.5f;  //3.85
	straightctrlpid.Ki = 0.02f; //0.02
	straightctrlpid.Kd = 0.3f;  // 0.08
	arm_pid_init_f32(&straightctrlpid, 1);	
	
	/* 轨迹速度控制PID参数 */
	straightspeedctrlpid.Kp = 0.0045f;
...
uint16_t MaxRunDis = 40; //开启轨迹控制算法左右最大距离 单位cm
uint16_t FMinRunDis = 30; //开启轨迹控制算法前方最小距离
...
#define TrunErrorFlag StraightCtrlFlag.flag4  // 转弯错误 需要校准

//#define TrunError // 开启转弯错误校正
int16_t outpidangle;   // PD 角度校正误差 //往左靠为正 往右靠为负
CCMRAM float pxs = 0.35f;
...
int16_t spdC_old; uint8_t TurnNum = 0;CCMRAM float anglecaltmp;
static void StraightCtrl(uint8_t distance)  // 轨迹控制 沿墙走 // 循迹
{
...
//		CENTER_disL = vl53l0x_dis3;
		//timeSaveAngle = osKernelSysTick();
		//static uint8_t delaynum ; //延迟等待低通滤波稳定
		//delaynum ++;
		//if(delaynum<100) return;
		//StraightCtrlFlag.flag = 0x00;
		SaveOldAngleFlag = 1;	
...
//					}
			...
			else  // 左右两边大于MaxRunDis 可能  遇到分叉口 
			{
					...
					if(osMutexWait(mototurnMute,0) == osOK)
					{
						...
						if((HAL_GetTick() - timeTurn) > 1000)
						{
							...
							//if(TurnNum == 2 )
								//spdC = 60;
							if(TurnNum == 4 || TurnNum == 8 || TurnNum == 12)
		...
		{
			//StartTrunFlag = 1;
			TrunErrorFlag = 1; //转弯错误
			spdC = -20;	
						
		}
		else
		{
			//if(TrunErrorFlag == 1) //转弯错误
			//{
			//if(StartTrunFlag == 0)
				//spdC = 30;	
			//}
		...
		angle_Turn = angle_Turn_old - LIMIT(outpidangle,-40,40);
		//static int8_t speed;
		
		/* 预减速 */ 
		if(CENTER_dis < FMinRunDis + 20 || TurnNum == 2 || TurnNum == 4 || TurnNum == 6 || TurnNum == 8 || TurnNum == 12)  // 预减速
		{				
	...
	else if(LEFT_dis < MaxRunDis && RIGHT_dis < MaxRunDis && CENTER_dis < FMinRunDis)// 三个距离都不符合 
	{
		if(StartRunFlag == 1&& TurnNum == 12)  // 可能走出跑道 //已经开始
		{
			...
			//spdC = 30;
			if(endtime>100) spdC = 0; //10 * 运行时间 ms后停车
			else endtime ++;
