...
CCMRAM float x,y,z;
CCMRAM float GYRO_Spx,GYRO_Spy,GYRO_Spz;
CCMRAM float Pitch,Roll,Yaw;
CCMRAM float HeadingDegrees;
CCMRAM float linear_accelX,linear_accelY,linear_accelZ;
CCMRAM float Gravity_X,Gravity_Y,Gravity_Z;
CCMRAM float Compass_X,Compass_Y,Compass_Z;
 /*  Pitch: -180 to 180
 *  Roll: -90 to 90
 *  Yaw: -180 to 180*/
...
static void handle_input(void)
{
    ...
    //HAL_UART_Receive(&huart4,&c,1,100);
	//SEGGER_RTT_WaitKey();
...
osThreadId mpu9250Handle;
osSemaphoreId INTReady;
...
void MPU9250FreertosInit(void)
{
	...
	osSemaphoreDef(INTReady);
	INTReady = osSemaphoreCreate(osSemaphore(INTReady),0);

   osThreadDef(MPU9250Task, MPU9250Task, osPriorityRealtime, 0, 512);
   mpu9250Handle = osThreadCreate(osThread(MPU9250Task), NULL);  
}
...
void mpu9250run(void)
{
	...
	unsigned char	new_temp = 0;
	unsigned long timestamp;
   osEvent mpu9250event;
	union 
  {
	 uint8_t mpl_states8[150];
	 uint16_t mpl_states16[75];
  }mpl_states;
  size_t num;
  uint16_t addr = FLASH_MEM_START;
	set = 0;
	//osDelay(100);
	dmp_set_fifo_rate(100);
	inv_set_quat_sample_rate(10000L);
 ...
 //#pragma arm section zidata = "ccmram"    
    ...
    int new_data = 0;
	  ...
	  if(set == 1)
	  {
        ...
        /* Let MPL know that contiguity was broken. */
		   inv_get_mpl_state_size(&store_size);
		  if (store_size/2 > FLASH_SIZE) {
		  }
		  else
		  {
			  if(store_size%2 == 0) num = store_size/2;
			  else num = store_size/2 +1;
				inv_save_mpl_states(mpl_states.mpl_states8, store_size); 
			  for(size_t i;i<num;i++) 
			  EE_WriteVariable(addr+i,mpl_states.mpl_states16[i]);
		  }
        inv_accel_was_turned_off();
        inv_gyro_was_turned_off();
        inv_compass_was_turned_off();
	  ...
	  else if(set == 2)
	  {
		  ...
		  if (store_size/2 > FLASH_SIZE) {
			...
			else
			{
				...
				for(size_t i;i<num;i++) EE_ReadVariable(addr+i,&mpl_states.mpl_states16[i]);
				inv_load_mpl_states(mpl_states.mpl_states8, store_size);
			  inv_accel_was_turned_off();
			  inv_gyro_was_turned_off();
			  inv_compass_was_turned_off();		  
		  }
   ...
   get_tick_count(&timestamp);
	  ...
	  /* Temperature data doesn't need to be read with every gyro sample.
		...
		*/
		...
		mpu9250event = osSignalWait(0x04,1);
		if(mpu9250event.status == osEventSignal)
		{
			
			if(mpu9250event.value.signals&0x01)
			{
				//timestamp++;
				//osDelay(1);
				//HAL_GPIO_TogglePin(D3_GPIO_Port,D3_Pin);
		...
		else if(mpu9250event.status == osEventTimeout)
		{
